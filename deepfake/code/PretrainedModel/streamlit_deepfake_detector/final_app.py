...
# Try to import our statistics helper
try:
    from statistics_helper import add_batch_testing_to_statistics
    STATS_HELPER_AVAILABLE = True
    print("Successfully imported statistics helper")
except ImportError:
    STATS_HELPER_AVAILABLE = False
    print("Statistics helper module not available")
...
def statistics_mode():
    ...
    # Create tabs for different statistics sections
    stats_tabs = st.tabs(["General Statistics", "Batch Testing Results", "Future Outlook"])
    ...
    # Tab 2: Batch Testing Results
    with stats_tabs[1]:
        # Add batch testing results if the module is available
        if STATS_HELPER_AVAILABLE:
            add_batch_testing_to_statistics()
        else:
            st.warning("Batch testing module not available. Please run the batch testing script first.")
            st.markdown("""
            To run the batch testing and view the results:
            
            1. Run the batch testing script:
            ```bash
            ...
            3. Restart the app to see the results.
            """)
